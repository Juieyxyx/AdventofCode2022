...
public class day7b {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //find the required dir size
        long deleteNum = 30000000 - Math.abs(70000000 - dic.get("/."));
        System.out.println("need to delete: " + deleteNum);
        var list = dic.values().stream().sorted().collect(Collectors.toList());
        list.add(deleteNum);
        list = list.stream().sorted().collect(Collectors.toList());
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) == deleteNum) {
                System.out.println("result = " +list.get(i + 1));
            }
        }

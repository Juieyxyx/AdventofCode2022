...
public class day22b {
    ...
    private static List<String> parseInstructionLine(String instructionLine) {
    ...
    private static void followInstructions(boolean part2) {
        current = new Coord(getIndexOfFirstNonEmptyXInRow(0), 0); // leftmost open tile of first row
        facing = Facing.RIGHT;

        for (String instruction : instructions) {
            try {
                Integer numSteps = Integer.parseInt(instruction);
    ...
    private static int getIndexOfFirstNonEmptyXInRow(int y) {
        char[] row = map[y];
    ...
    private static int getIndexOfLastNonEmptyXInRow(int y) {
        char[] row = map[y];
    ...
    private static int getIndexOfFirstNonEmptyYInColumn(int x) {
    ...
    private static int getIndexOfLastNonEmptyYInColumn(int x) {
    ...
    private static Coord getCoordOfNextMove(boolean part2) {
        int cX = current.x();
        int cY = current.y();
    ...
    private static List<Object> nextCoordWhenMovingUpOnCube(int x, int y) { // UP
        if (onA(x, y)) {
            // fine, moving into B
    ...
    private static List<Object> nextCoordWhenMovingDownOnCube(int x, int y) { // DOWN
        if (onA(x, y)) {
    ...
    private static List<Object> nextCoordWhenMovingLeftOnCube(int x, int y) { // LEFT
        if (onA(x, y)) {
    ...
    private static List<Object> nextCoordWhenMovingRightOnCube(int x, int y) { // RIGHT
        if (onA(x, y)) {
    ...
    private static boolean onA(int x, int y) {
        return (
                (x >= 0 && x <= (FACE_SIZE - 1)) &&
    ...
    private static boolean onB(int x, int y) {
        return (
                (x >= 0 && x <= (FACE_SIZE - 1)) &&
    ...
    private static boolean onC(int x, int y) {
        return (
    ...
    private static boolean onD(int x, int y) {
        return (
                (x >= FACE_SIZE && x <= ((FACE_SIZE * 2) - 1)) &&
    ...
    private static boolean onE(int x, int y) {
        return (
                (x >= FACE_SIZE && x <= ((FACE_SIZE * 2) - 1)) &&
                        (y >= 0 && y <= (FACE_SIZE - 1))
        );
    ...
    private static Facing turn(String instruction) {
    ...
    private static boolean isWall(Coord coord) {
